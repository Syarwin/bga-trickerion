...
export class LearnTrick {
    ...
    onEnteringState(args: LearnTrickArgs, isCurrentPlayerActive: boolean) {
        if (isCurrentPlayerActive) {
            for (const trick of args.availableTricks) {
                this.bga.statusBar.addActionButton(trick.type, () => this.bga.actions.performAction("actLearnTrick", { trickId: trick.id }));
            }
        }
    }
     ...
     */
    onLeavingState(args: LearnTrickArgs, isCurrentPlayerActive: boolean) {
    }
