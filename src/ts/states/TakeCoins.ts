...
export class TakeCoins {
    ...
    onEnteringState(args: TakeCoinsArgs, isCurrentPlayerActive: boolean) {
        if (isCurrentPlayerActive) {
            for (const coins of args.availableCoins) {
                this.bga.statusBar.addActionButton(`get ${coins} coins`, () => this.bga.actions.performAction("actTakeCoins", { coins }));
            }
        }
    }
     ...
     */
    onLeavingState(args: TakeCoinsArgs, isCurrentPlayerActive: boolean) {
    }
