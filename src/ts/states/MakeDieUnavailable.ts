...
export class MakeDieUnavailable {
    ...
    onEnteringState(args: MakeDieUnavailableArgs, isCurrentPlayerActive: boolean) {
        if (isCurrentPlayerActive) {
            for (const die of args.availableDice) {
                this.bga.statusBar.addActionButton(die.toString(), () => this.bga.actions.performAction("actMakeDieUnavailable", { dieFace: die }));
            }
        }
    }
     ...
     */
    onLeavingState(args: MakeDieUnavailableArgs, isCurrentPlayerActive: boolean) {
    }
