...
export class SetDie {
        ...
        if (isCurrentPlayerActive) {
            for (const dieType in args.availableDice) {
                ...
                for (let i=0; i < args.availableDice[dieType].length; i++) {
                    ...
                    this.bga.statusBar.addActionButton(`${dieType}: ${dieFace}`, () => {
                        this.bga.statusBar.removeActionButtons();
                        for (const dieFace of args.availableFaces[dieType][i]) {
                            this.bga.statusBar.addActionButton(dieFace.toString(), () => this.bga.actions.performAction("actSetDie", { dieType, dieId: i, dieFace }));
                        }                        
     ...
     */
    onLeavingState(args: SetDieArgs, isCurrentPlayerActive: boolean) {
    }
